...
function f(e, t, n, r) {
	let i = t / 2, a = n / 2, o = Math.abs(r) * Math.PI / 180, s = e * Math.cos(o), c = -e * Math.sin(o), l = Math.min(-i, s - i, (e - i) * Math.cos(o)), u = e + i, d = Math.min(-e - i, c - i, -Math.sin(o) * (e + i)), f = i;
	return {
		min: [
			l,
			-a,
...
function p(e, t, n, r, i) {
	let a = t / 2, o = n / 2, s = Math.abs(r) * Math.PI / 180, c = e * Math.cos(s), l = (i ? -1 : 1) * e * Math.sin(s), u = Math.min(-o, c - o, (e - o) * Math.cos(s)), d = e + o, f = i ? Math.min(-e - o, l - o) : -o, p = i ? o : Math.max(e + o, l + o);
	return {
		min: [
			u,
			f,
...
var m = class {
	...
	static register(t) {
		...
		t.getBounds = (r) => {
			try {
				let i = t.buildGeometry(r);
				i.updateMatrixWorld(!0);
				let a = new e.Box3().setFromObject(i), o = n ? n(r) : void 0, s = (e) => Math.round(e * 1e3 * 100) / 100 + 0;
				return {
					min: [
						s(a.min.x),
						s(a.min.y),
					...
					clearanceEnvelope: o?.clearanceEnvelope
				};
			} catch {
				return n(r);
			}
		}, this._definitions.set(t.id, t);
	}
	static initAll() {
		this._definitions.clear(), this.register({
			...
			getBounds: (e) => {
				let t = e.inletWidth || 600, n = (e.depth || 100) / 2, r = (e.length || 500) / 2;
				return {
					min: [
						-t / 2,
						-n,
	...
	static computeMateTransform(t, n, r, i, a = .5) {
		...
		let c = new _(o.worldPosition, o.worldDirection, o.worldUp), l = c.direction.clone().negate().normalize(), u = c.up.clone().normalize(), d = new e.Vector3().crossVectors(u, l).normalize(), f = new e.Vector3().crossVectors(l, d).normalize(), p = new e.Matrix4();
		p.makeBasis(d, f, l);
		let m = new _(s.localPosition, s.localDirection, s.localUp), h = new e.Matrix4();
		h.makeBasis(m.right, m.up, m.direction);
		let g = h.clone().transpose(), v = new e.Matrix4().multiplyMatrices(p, g), y = new e.Quaternion().setFromRotationMatrix(v), b = m.position.clone().applyQuaternion(y), x = c.position.clone().sub(b), S = {
			position: [
				x.x,
				x.y,
				x.z
		...
		}, C = m.position.clone().applyQuaternion(y).add(x), w = m.direction.clone().applyQuaternion(y).normalize(), T = C.distanceTo(c.position), E = w.dot(c.direction), D = T <= a && E <= -.99;
		return {
			success: D,
			placement: S,
			positionErrorMm: T,
	...
	static generateBom(e, t = {}) {
		...
		e.forEach((e) => {
			(g.isAssembly(e.definitionId) || e.definition.subComponents && e.definition.subComponents.length > 0) && (c.add(e.instanceId), (g.getSubComponents(e.definitionId).length > 0 ? g.getSubComponents(e.definitionId) : e.definition.subComponents || []).forEach((t) => {
		...
		e.forEach((e) => {
			...
			let h = g.isAssembly(t.id) || t.subComponents && t.subComponents.length > 0 || !1;
	...
	static testCaseA() {
		let e = m.get("TRAY_STRAIGHT"), t = m.get("FITTING_ELBOW_90"), n = new s("inst_straight", e, {
			width: 600,
			depth: 100
		}), r = new s("inst_elbow", t, {
			width: 600,
			depth: 100
		}), i = y.validateConnection(n, "PORT_B", r, "PORT_A"), a = i.valid && i.code === "OK";
		return {
			id: "Case A",
	...
	static testCaseB() {
		let e = m.get("TRAY_STRAIGHT"), t = m.get("FITTING_ELBOW_90"), n = new s("inst_straight", e, {
			...
			width: 450,
			depth: 100
		}), i = y.validateConnection(n, "PORT_B", r, "PORT_A"), a = !i.valid && (i.code === "WIDTH_MISMATCH" || i.code === "DIMENSION_MISMATCH");
		return {
			id: "Case B",
	...
	static testCaseC() {
		let e = m.get("TRAY_STRAIGHT"), t = m.get("FITTING_ELBOW_90"), n = new s("inst_straight", e, {
			...
			width: 600,
			depth: 150
		}), i = y.validateConnection(n, "PORT_B", r, "PORT_A"), a = !i.valid && (i.code === "DEPTH_MISMATCH" || i.code === "DIMENSION_MISMATCH");
		return {
			id: "Case C",
	...
	static testCaseD() {
		let e = m.get("TRAY_STRAIGHT"), t = m.get("EQUIP_JUNCTION_BOX"), n = new s("inst_straight", e), r = new s("inst_jb", t), i = y.validateConnection(n, "PORT_B", r, "PORT_BOTTOM_GLAND"), a = !i.valid && i.code === "TYPE_MISMATCH";
		return {
			id: "Case D",
	...
	static testCaseE() {
		let e = m.get("TRAY_STRAIGHT"), t = m.get("TRAY_STRAIGHT"), n = m.get("FITTING_REDUCER_LEFT"), r = new s("s600", e, {
			width: 600,
			depth: 100
		}), i = new s("red", n, {
			inletWidth: 600,
			...
			width: 450,
			depth: 100
		}), o = y.validateConnection(r, "PORT_B", i, "PORT_A"), c = y.validateConnection(i, "PORT_B", a, "PORT_A"), l = o.valid && c.valid;
		return {
			id: "Case E",
	...
	static testCaseF() {
		let e = m.get("TRAY_STRAIGHT"), t = new s("instA", e, { length: 3e3 }), n = new s("instB", e, { length: 3e3 }), r = v.computePlacement(t, "PORT_B", n, "PORT_A");
		n.setPlacement(r);
		let i = t.getWorldPorts().find((e) => e.id === "PORT_B"), a = n.getWorldPorts().find((e) => e.id === "PORT_A"), c = o.distance(i.worldPosition, a.worldPosition), l = o.dot(i.worldDirection, a.worldDirection), u = o.dot(i.worldUp, a.worldUp), d = c <= .001, f = Math.abs(l - -1) <= .001, p = Math.abs(u - 1) <= .001, h = d && f && p;
		return {
			id: "Case F",
			...
			actual: h ? `PASS (Dist=${c.toFixed(4)}mm, DirDot=${l.toFixed(3)}, UpDot=${u.toFixed(3)})` : `FAIL: Dist=${c}mm, DirDot=${l}, UpDot=${u}`,
			details: {
				dist: c,
				dotDir: l,
				dotUp: u,
	...
	static testCaseG() {
		let e = m.get("TRAY_STRAIGHT"), t = [], n = new s("tray_0", e, { length: 3e3 });
		n.setPlacement({
			position: [
				0,
				0,
		...
		for (let n = 1; n < 10; n++) {
			let r = t[n - 1], i = new s(`tray_${n}`, e, { length: 3e3 }), a = v.computePlacement(r, "PORT_B", i, "PORT_A");
	...
	static testCaseH() {
		let e = m.get("FITTING_ELBOW_90"), t = x.exportDefinition(e), n = t.schemaVersion === "2.0.0", r = t.ports.length === 2, i = t.centerlines.length === 1, a = !!t.bounds && t.bounds.min && t.bounds.max, o = n && r && i && a;
		return {
			id: "Case H",
	...
	static testCaseI() {
		let e = m.get("FITTING_ELBOW_90"), t = new s("e1", e, { radius: 600 }), n = new s("e2", e, { radius: 600 }), r = new s("e3", e, { radius: 600 }), i = new s("e4", e, { radius: 600 });
		n.setPlacement(v.computePlacement(t, "PORT_B", n, "PORT_A")), r.setPlacement(v.computePlacement(n, "PORT_B", r, "PORT_A")), i.setPlacement(v.computePlacement(r, "PORT_B", i, "PORT_A"));
	...
	static testCaseJ() {
		let e = m.get("OBSTACLE_MAIN_PROCESS_PIPE").getBounds({
			diameterMm: 500,
			lengthMm: 6e3,
			clearanceMm: 150
	...
	static testCaseK() {
		let e = h.checkConformance({ width: 600 }), t = h.checkConformance({ width: 550 }), n = e.isStandard && !t.isStandard && t.warnings.length > 0;
		return {
			id: "Case K",
	...
	static testCaseL() {
		let e = m.get("TRAY_STRAIGHT"), t = m.get("FITTING_ELBOW_90"), n = m.get("FITTING_ELBOW_45"), r = e.getCenterlineRoutes({ length: 3e3 })[0], i = t.getCenterlineRoutes({
			radius: 600,
			angleDeg: 90
	...
	static testCaseN() {
		...
		n.forEach((n) => {
			...
			let f = d.clone().project(t), p = [
				...
				+u.max.y.toFixed(4),
				+u.max.z.toFixed(4)
			], h = [+f.x.toFixed(4), +f.y.toFixed(4)], g = Math.abs(c - a.vertexCount), _ = Math.abs(l - a.triangleCount), v = Math.abs(s.children.length - a.childMeshCount), y = p.reduce((e, t, n) => Math.max(e, Math.abs(t - a.bounds[n])), 0), b = Math.max(Math.abs(h[0] - a.projectedCentroid[0]), Math.abs(h[1] - a.projectedCentroid[1])), x = g + _ + v + +(y > .005) + +(b > .005);
			r += x, i.push({
				id: n,
				baseline: a,
				actual: {
	...
	static testCaseO() {
		let t = 37.5, n = new s("inst_custom_angle", m.get("FITTING_ELBOW_90"), {
			angleDeg: t,
			radius: 600,
			width: 600
		}), r = n.getWorldPorts(), i = r.find((e) => e.id === "PORT_A"), a = r.find((e) => e.id === "PORT_B"), c = n.getCenterlines()[0], l = n.getThreeMesh(), u = t * Math.PI / 180, d = 600 * Math.cos(u), f = -600 * Math.sin(u), p = 600 * u, g = Math.hypot(a.worldPosition[0] - d, a.worldPosition[2] - f), _ = g < .01, v = c.samplePoints[0], y = c.samplePoints[c.samplePoints.length - 1], b = o.distance(v, i.worldPosition), x = o.distance(y, a.worldPosition), S = b < .01 && x < .01, C = Math.abs(c.analyticLength - p), w = C < .01, T = new e.Box3().setFromObject(l), E = T.min.z < 0 && T.max.z > 0, D = h.checkConformance({ angleDeg: t }), O = !D.isStandard && D.warnings.some((e) => e.includes("Non-standard")), k = _ && S && w && E && O;
		return {
			id: "Case O",
			...
			actual: k ? `PASS (37.5°: Port B=[${a.worldPosition[0].toFixed(1)}, ${a.worldPosition[2].toFixed(1)}]mm, CenterlineErr=${x.toFixed(3)}mm, Length=${c.analyticLength.toFixed(2)}mm, 型錄警示確認)` : `FAIL: portPassed=${_}, clPassed=${S}, lenPassed=${w}, warns=${O}`,
			details: {
				portBPosErr: g,
				clStartErr: b,
				clEndErr: x,
	...
	static testCaseP() {
		let e = new s("inst_consistency", m.get("FITTING_ELBOW_90"), {
			radius: 300,
			angleDeg: 45
	...
	static testCaseR_BoundsConsistency() {
		...
		t.forEach((t) => {
			...
			let o = a.getBounds(a.defaultParameters), s = a.buildGeometry(a.defaultParameters);
			s.updateMatrixWorld(!0);
			let c = new e.Box3().setFromObject(s), l = [
				c.min.x * 1e3,
				c.min.y * 1e3,
				c.min.z * 1e3
			], u = [
			...
			for (let e = 0; e < 3; e++) {
				let a = Math.abs(o.min[e] - l[e]);
				r = Math.max(r, a), a > i && n.push({
					id: t,
					axis: `min${d[e]}`,
					boundsVal: o.min[e],
					geoVal: l[e],
		...
		return {
			...
			passed: a,
			expected: `All ${t.length} required components have minX/Y/Z & maxX/Y/Z matching THREE.Box3 within ${i}mm`,
			actual: a ? `PASS (已嚴格比對 ${t.length} 組構件之 6 軸包絡邊界 min/max，最大誤差 = ${r.toExponential(2)}mm)` : `FAIL: ${n.length} bounding coordinate inconsistencies (Max Error = ${r.toFixed(3)}mm)`,
			details: {
				requiredTestIds: t,
				maxError: r,
				failures: n
	...
	static testCaseS_PortFrameHandedness() {
		let e = m.getAll(), t = 0, n = 1, r = 1, i = [];
		e.forEach((e) => {
			e.getLocalPorts(e.defaultParameters).forEach((a) => {
				t++;
				let o = new _(a.localPosition, a.localDirection, a.localUp).getDeterminant();
	...
	static testCaseT_EccentricReducerLength() {
		let e = m.get("FITTING_REDUCER_LEFT"), t = m.get("FITTING_REDUCER_CENTER"), n = e.getCenterlineRoutes({
			inletWidth: 600,
			outletWidth: 450,
			length: 500
	...
	static testCaseU_TeePhysicalCenterline() {
		let e = m.get("FITTING_TEE").getCenterlineRoutes({
			length: 1400,
			branchLength: 700,
			radius: 300
	...
	static testCaseV_DerivedStateExport() {
		let e = new s("test_export_inst", m.get("TRAY_STRAIGHT")), t = x.exportScene([e]).instances[0], n = !!t.derivedSnapshot, r = t.derivedSnapshot?.status === "NON_CANONICAL_SNAPSHOT", i = !!t.effectiveParameters && !!t.placement, a = n && r && i;
		return {
			id: "Case V",
...
//#endregion
export { C as AcceptanceTestSuite, c as AnalyticLength, g as AssemblyRegistry, t as AssumptionLevel, b as BomManager, n as BomScope, h as CatalogStandards, s as ComponentInstance, r as ComponentOrigin, m as ComponentRegistry, i as ComponentRole, y as ConnectionValidator, d as GeometryGenerators, x as JsonExporter, S as LEGACY_FITTING_BASELINES, v as MateEngine, u as Materials, l as RouteGenerator, o as Transforms, a as Units, w as VERSION };
