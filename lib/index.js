...
var m = class {
	...
	static testCaseR_BoundsConsistency() {
		...
		t.forEach((t) => {
			let r = m.get(t);
			if (!r) return;
			let i = r.getBounds(r.defaultParameters), a = r.buildGeometry(r.defaultParameters);
			a.updateMatrixWorld(!0);
			let o = new e.Box3().setFromObject(a), s = [
				o.min.x * 1e3,
				o.min.y * 1e3,
				o.min.z * 1e3
			], c = [
			...
			if (t === "STRUCT_COLUMN") {
				let e = Math.abs(c[1] - i.max[1]), r = Math.abs(s[1] - i.min[1]);
				(e > 1 || r > 1) && n.push({
					id: t,
					hErr: e,
					yBaseErr: r,
		...
		return {
			...
			actual: r ? "PASS (結構柱高 EL +8.0m 與底座 Y=0 實體網格與 getBounds 範圍完全吻合)" : "FAIL: Bounding inconsistencies found",
			details: n
		};
	}
