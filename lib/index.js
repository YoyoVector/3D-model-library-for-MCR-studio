...
var t = {
	...
	static createHorizontalElbow(e, t, n, r, i = 20, a = 0) {
		...
		return {
			...
			toPort: t,
			type: "ARC_XZ",
			analyticLength: a > 0 ? c.arcWithTangents(n, r, a) : c.circularArc(n, r),
			samplePoints: s
		};
	}
	...
	static buildHorizontalCross(t) {
		let n = new e.Group(), r = a.mmToM(t.width), i = a.mmToM(t.depth), o = t.radius ?? 300, s = t.tangentLength ?? 125, c = t.length ?? t.width + 2 * o + 2 * s, l = a.mmToM(c), d = .04, f = t.isIS ? u.TrayIS : u.Tray, p = l / 2, m = p - r / 2;
		[
			{
				sx: 1,
				sz: 1
			...
			let o = new e.Mesh(new e.BoxGeometry(m, i, d), f);
			o.position.set(t * (r / 2 + m / 2), 0, r / 2 * a), o.castShadow = !0, n.add(o);
			let s = new e.Mesh(new e.BoxGeometry(d, i, m), f);
			s.position.set(r / 2 * t, 0, a * (r / 2 + m / 2)), s.castShadow = !0, n.add(s);
		});
		let h = .02, g = .035;
		for (let t = r / 2 + .1; t <= p - .05; t += .2) [-1, 1].forEach((a) => {
			let o = new e.Mesh(new e.BoxGeometry(g, h, r), f);
			o.position.set(a * t, -i / 2 + h / 2, 0), o.castShadow = !0, n.add(o);
		});
		for (let t = r / 2 + .1; t <= p - .05; t += .2) [-1, 1].forEach((a) => {
			let o = new e.Mesh(new e.BoxGeometry(r, h, g), f);
			o.position.set(0, -i / 2 + h / 2, a * t), o.castShadow = !0, n.add(o);
		});
		return n;
	}
};
...
var g = class {
	...
	static initAll() {
		this._definitions.clear(), this.register({
			...
			provenance: { radius: {
				source: "Vendor Catalog Page 7 & Page 42-43 R=600mm",
				assumptionLevel: t.VERIFIED_VENDOR_CATALOG,
				notes: "60° Standard Elbow"
			} },
			getLocalPorts: (e) => {
				let t = e.radius || 600, n = e.width || 600, r = e.depth || 100, i = (e.angleDeg ?? 60) * Math.PI / 180, a = t * Math.cos(i), o = -t * Math.sin(i), s = -Math.sin(i), c = -Math.cos(i);
				return [{
					id: "PORT_A",
					name: "Inlet Port A",
					localPosition: [
			...
			getCenterlineRoutes: (e) => [l.createHorizontalElbow("PORT_A", "PORT_B", e.radius || 600, e.angleDeg ?? 60)],
			getBounds: (e) => f(e.radius || 600, e.width || 600, e.depth || 100, e.angleDeg ?? 60),
			buildGeometry: (e) => d.buildHorizontalElbow({
				...e,
				angleDeg: e.angleDeg ?? 60
			})
		}), this.register({
			schemaVersion: "2.0.0",
			componentVersion: "1.0.0",
			...
			provenance: { radius: {
				source: "Vendor Catalog Page 7 & Page 43 R=600mm",
				assumptionLevel: t.VERIFIED_VENDOR_CATALOG,
				notes: "30° Standard Elbow"
			} },
			getLocalPorts: (e) => {
				let t = e.radius || 600, n = e.width || 600, r = e.depth || 100, i = (e.angleDeg ?? 30) * Math.PI / 180, a = t * Math.cos(i), o = -t * Math.sin(i), s = -Math.sin(i), c = -Math.cos(i);
				return [{
					id: "PORT_A",
					name: "Inlet Port A",
					localPosition: [
			...
			getCenterlineRoutes: (e) => [l.createHorizontalElbow("PORT_A", "PORT_B", e.radius || 600, e.angleDeg ?? 30)],
			getBounds: (e) => f(e.radius || 600, e.width || 600, e.depth || 100, e.angleDeg ?? 30),
			buildGeometry: (e) => d.buildHorizontalElbow({
				...e,
				angleDeg: e.angleDeg ?? 30
			})
		}), this.register({
			schemaVersion: "2.0.0",
			componentVersion: "1.0.0",
			...
			provenance: {
				width: {
					source: "Catalog Page 10 (CNS 13303 / NEMA VE 1 Aluminum Cable Tray)",
					assumptionLevel: t.VERIFIED_VENDOR_CATALOG,
					notes: "Standard widths 100~1000mm"
				},
				depth: {
					source: "Catalog Page 10 side rail height H=150mm",
					assumptionLevel: t.VERIFIED_VENDOR_CATALOG,
					notes: "Standard 100/150mm depth"
				},
				length: {
					source: "Catalog Page 10: Span = W + 2*R + 250mm",
					assumptionLevel: t.VERIFIED_VENDOR_CATALOG,
					notes: "125mm tangent extension at all 4 ports"
				},
				radius: {
					source: "Catalog Page 10 bend fillet R=300/600/900mm",
					assumptionLevel: t.VERIFIED_VENDOR_CATALOG,
					notes: "R=300mm corner radius"
				}
			},
			getLocalPorts: (e) => {
				let t = e.width || 600, n = e.depth || 100, r = e.radius || 300, i = e.tangentLength ?? 125, a = (e.length || t + 2 * r + 2 * i) / 2;
				return [
					{
						id: "PORT_A",
						name: "Inlet Port A (-X)",
			...
			getCenterlineRoutes: (e) => {
				let t = e.width || 600, n = e.radius || 300, r = e.tangentLength ?? 125, i = e.length || t + 2 * n + 2 * r;
				return [
					l.createStraightX("PORT_A", "PORT_B", i),
					l.createStraightZ("PORT_D", "PORT_C", i),
					l.createCrossCornerRoute("PORT_A", "PORT_C", i, n, -1, 1),
					l.createCrossCornerRoute("PORT_A", "PORT_D", i, n, -1, -1),
					l.createCrossCornerRoute("PORT_B", "PORT_C", i, n, 1, 1),
					l.createCrossCornerRoute("PORT_B", "PORT_D", i, n, 1, -1)
				];
			},
			getBounds: (e) => {
				let t = e.width || 600, n = e.depth || 100, r = e.radius || 300, i = e.tangentLength ?? 125, a = (e.length || t + 2 * r + 2 * i) / 2;
			...
			buildGeometry: (e) => d.buildHorizontalCross(e)
		}), this.register({
			schemaVersion: "2.0.0",
			componentVersion: "1.0.0",
			id: "STRUCT_MAIN_BAY",
...
function x(e, t, n = {}, r) {
	...
	let a = r || `${e.toLowerCase()}_${Date.now()}_${Math.floor(Math.random() * 1e3)}`, o = {
		width: t.width,
		depth: t.height,
	...
	switch (e) {
		...
		case "TRAY_STRAIGHT_DIVIDER":
			o.length = n.length ?? t.standardLength, t.rungSpacingMm && (o.rungSpacing = t.rungSpacingMm);
			break;
		case "FITTING_ELBOW_90":
		case "FITTING_ELBOW_45":
			o.radius = n.radius ?? t.defaultRadius, o.angleDeg = n.angleDeg ?? (e === "FITTING_ELBOW_45" ? 45 : 90), o.tangentLength = n.tangentLength ?? t.tangentLength;
			break;
		case "FITTING_RISER_IN_90":
		case "FITTING_RISER_IN_45":
		case "FITTING_RISER_OUT_90":
		case "FITTING_RISER_OUT_45":
			o.radius = n.radius ?? t.defaultRadius, o.angleDeg = n.angleDeg ?? (e.endsWith("45") ? 45 : 90), o.tangentLength = n.tangentLength ?? t.tangentLength;
			break;
		case "FITTING_TEE": {
			let e = n.radius ?? t.defaultRadius, r = n.tangentLength ?? t.tangentLength;
			o.radius = e, o.tangentLength = r, o.length = n.length ?? t.width + 2 * e + 2 * r, o.branchLength = n.branchLength ?? t.width / 2 + e + r;
			break;
		}
		case "FITTING_CROSS": {
			let e = n.radius ?? t.defaultRadius, r = n.tangentLength ?? t.tangentLength;
			o.radius = e, o.tangentLength = r, o.length = n.length ?? t.width + 2 * e + 2 * r;
			break;
		}
		case "FITTING_REDUCER_CENTER":
		case "FITTING_REDUCER_LEFT":
		case "FITTING_REDUCER_RIGHT": o.inletWidth = n.inletWidth ?? t.width, o.outletWidth = n.outletWidth ?? Math.max(100, t.width / 2), o.length = n.length ?? 600, o.tangentLength = n.tangentLength ?? 200;
	}
	return Object.assign(o, n), new s(a, i, o);
}
//#endregion
...
function C(e) {
	return e.startsWith("TRAY_STRAIGHT") ? "STRAIGHT" : e.includes("ELBOW") ? "ELBOW" : e.includes("TEE") || e.includes("CROSS") ? "BRANCH" : e.includes("RISER") ? "RISER" : e.includes("REDUCER") ? "REDUCER" : e.startsWith("ACCESSORY") ? "ACCESSORY" : e.startsWith("STRUCT") ? "STRUCTURAL" : e.startsWith("EQUIP") || e.startsWith("PENETRATION") || e.startsWith("CONTEXT") ? "EQUIPMENT" : e.startsWith("OBSTACLE") ? "OBSTACLE" : "ACCESSORY";
}
var w = {
...
function T(e) {
	...
	let t = ["SYSTEM_LADDER"];
	return (e === "TRAY_STRAIGHT" || e === "FITTING_ELBOW_90" || e === "FITTING_ELBOW_45" || e === "FITTING_TEE" || e === "FITTING_RISER_IN_90" || e === "FITTING_RISER_OUT_90" || e === "FITTING_REDUCER_CENTER") && t.push("SYSTEM_VENTILATED_SMALL"), (e === "TRAY_STRAIGHT" || e === "FITTING_ELBOW_90" || e === "FITTING_ELBOW_60" || e === "FITTING_ELBOW_45" || e === "FITTING_ELBOW_30" || e === "FITTING_TEE" || e === "FITTING_CROSS" || e === "FITTING_RISER_IN_90" || e === "FITTING_RISER_OUT_90" || e === "FITTING_REDUCER_CENTER" || e === "FITTING_REDUCER_LEFT" || e === "FITTING_REDUCER_RIGHT") && t.push("SYSTEM_VENTILATED_LARGE"), t;
}
//#endregion
...
var E = class {
	...
	static computeMateTransform(t, n, r, i, a = .5) {
		...
		let c = new E(o.worldPosition, o.worldDirection, o.worldUp), l = c.direction.clone().negate().normalize(), u = c.up.clone().normalize(), d = new e.Vector3().crossVectors(u, l).normalize(), f = new e.Vector3().crossVectors(l, d).normalize(), p = new e.Matrix4();
		p.makeBasis(d, f, l);
		let m = new E(s.localPosition, s.localDirection, s.localUp), h = new e.Matrix4();
		...
		}, S = m.position.clone().applyQuaternion(v).add(b), C = m.direction.clone().applyQuaternion(v).normalize(), w = S.distanceTo(c.position), T = C.dot(c.direction), D = w <= a && T <= -.99;
		return {
			success: D,
			placement: x,
			positionErrorMm: w,
			alignmentDotProduct: T,
			message: D ? `Mated successfully. Position error: ${w.toFixed(4)} mm, alignment dot: ${T.toFixed(4)}` : `Mating out of tolerance. Error: ${w.toFixed(4)} mm, dot: ${T.toFixed(4)}`
		};
	}
...
var j = class {
	...
	static runAll() {
		g.initAll();
		let e = [];
		e.push(this.testCaseA()), e.push(this.testCaseB()), e.push(this.testCaseC()), e.push(this.testCaseD()), e.push(this.testCaseE()), e.push(this.testCaseF()), e.push(this.testCaseG()), e.push(this.testCaseH()), e.push(this.testCaseI()), e.push(this.testCaseJ()), e.push(this.testCaseK()), e.push(this.testCaseL()), e.push(this.testCaseM()), e.push(this.testCaseN()), e.push(this.testCaseO()), e.push(this.testCaseP()), e.push(this.testCaseQ_CenterlinePortEndpoints()), e.push(this.testCaseR_BoundsConsistency()), e.push(this.testCaseS_PortFrameHandedness()), e.push(this.testCaseT_EccentricReducerLength()), e.push(this.testCaseU_TeePhysicalCenterline()), e.push(this.testCaseV_DerivedStateExport()), e.push(this.testCaseW_CatalogProfileA()), e.push(this.testCaseX_CatalogProfileB()), e.push(this.testCaseY_HorizontalCross()), e.push(this.testCaseZ_GenericAngleVendor()), e.push(this.testCaseAA_VendorDimensionFormulas()), e.push(this.testCaseAB_ProfileConsistencyInvariant());
	...
	static testCaseA() {
		let e = g.get("TRAY_STRAIGHT"), t = g.get("FITTING_ELBOW_90"), n = new s("inst_straight", e, {
			...
			width: 600,
			depth: 100
		}), i = O.validateConnection(n, "PORT_B", r, "PORT_A"), a = i.valid && i.code === "OK";
		return {
			id: "Case A",
	...
	static testCaseB() {
		let e = g.get("TRAY_STRAIGHT"), t = g.get("FITTING_ELBOW_90"), n = new s("inst_straight", e, {
			...
			width: 450,
			depth: 100
		}), i = O.validateConnection(n, "PORT_B", r, "PORT_A"), a = !i.valid && (i.code === "WIDTH_MISMATCH" || i.code === "DIMENSION_MISMATCH");
		return {
			id: "Case B",
	...
	static testCaseC() {
		let e = g.get("TRAY_STRAIGHT"), t = g.get("FITTING_ELBOW_90"), n = new s("inst_straight", e, {
			...
			width: 600,
			depth: 150
		}), i = O.validateConnection(n, "PORT_B", r, "PORT_A"), a = !i.valid && (i.code === "DEPTH_MISMATCH" || i.code === "DIMENSION_MISMATCH");
		return {
			id: "Case C",
	...
	static testCaseD() {
		let e = g.get("TRAY_STRAIGHT"), t = g.get("EQUIP_JUNCTION_BOX"), n = new s("inst_straight", e), r = new s("inst_jb", t), i = O.validateConnection(n, "PORT_B", r, "PORT_BOTTOM_GLAND"), a = !i.valid && i.code === "TYPE_MISMATCH";
		return {
			id: "Case D",
	...
	static testCaseE() {
		let e = g.get("TRAY_STRAIGHT"), t = g.get("TRAY_STRAIGHT"), n = g.get("FITTING_REDUCER_LEFT"), r = new s("s600", e, {
			...
			width: 450,
			depth: 100
		}), o = O.validateConnection(r, "PORT_B", i, "PORT_A"), c = O.validateConnection(i, "PORT_B", a, "PORT_A"), l = o.valid && c.valid;
		return {
			id: "Case E",
	...
	static testCaseF() {
		let e = g.get("TRAY_STRAIGHT"), t = new s("instA", e, { length: 3e3 }), n = new s("instB", e, { length: 3e3 }), r = D.computePlacement(t, "PORT_B", n, "PORT_A");
		n.setPlacement(r);
	...
	static testCaseG() {
		...
		for (let n = 1; n < 10; n++) {
			let r = t[n - 1], i = new s(`tray_${n}`, e, { length: 3e3 }), a = D.computePlacement(r, "PORT_B", i, "PORT_A");
	...
	static testCaseH() {
		let e = g.get("FITTING_ELBOW_90"), t = j.exportDefinition(e), n = t.schemaVersion === "2.0.0", r = t.ports.length === 2, i = t.centerlines.length === 1, a = !!t.bounds && t.bounds.min && t.bounds.max, o = n && r && i && a;
		return {
			id: "Case H",
	...
	static testCaseI() {
		...
		n.setPlacement(D.computePlacement(t, "PORT_B", n, "PORT_A")), r.setPlacement(D.computePlacement(n, "PORT_B", r, "PORT_A")), i.setPlacement(D.computePlacement(r, "PORT_B", i, "PORT_A"));
	...
	static testCaseR_BoundsConsistency() {
		...
		e.forEach((e) => {
			...
			let a = i.getBounds(i.defaultParameters), o = A(i, i.defaultParameters), s = o.min, c = o.max, l = [
				"X",
				"Y",
				"Z"
			];
	...
	static testCaseS_PortFrameHandedness() {
		...
		e.forEach((e) => {
			e.getLocalPorts(e.defaultParameters).forEach((a) => {
				t++;
				let o = new E(a.localPosition, a.localDirection, a.localUp).getDeterminant();
	...
	static testCaseV_DerivedStateExport() {
		let e = new s("test_export_inst", g.get("TRAY_STRAIGHT")), t = j.exportScene([e]).instances[0], n = !!t.derivedSnapshot, r = t.derivedSnapshot?.status === "NON_CANONICAL_SNAPSHOT", i = !!t.effectiveParameters && !!t.placement, a = n && r && i;
		return {
			id: "Case V",
	...
	static testCaseW_CatalogProfileA() {
		let e = b.get("VENTILATED_PROFILE_A");
		if (!e) return {
			id: "Case W",
		...
		let t = x("TRAY_STRAIGHT", e), n = x("FITTING_ELBOW_90", e);
		x("FITTING_ELBOW_90", e, { angleDeg: 45 });
		let r = x("FITTING_TEE", e), i = t.effectiveParameters.width === 100, a = t.effectiveParameters.depth === 50, s = n.effectiveParameters.width === 100, c = n.effectiveParameters.depth === 50, l = n.effectiveParameters.radius === 300, u = n.effectiveParameters.tangentLength === 125, d = r.effectiveParameters.width === 100, f = r.effectiveParameters.depth === 50, p = O.validateConnection(t, "PORT_B", n, "PORT_A"), m = D.computePlacement(t, "PORT_B", n, "PORT_A");
		n.setPlacement(m);
		let h = t.getWorldPorts().find((e) => e.id === "PORT_B"), g = n.getWorldPorts().find((e) => e.id === "PORT_A"), _ = o.distance(h.worldPosition, g.worldPosition), v = i && a && s && c && l && u && d && f && p.valid && _ < .001;
		return {
			id: "Case W",
			...
			actual: v ? `PASS (Profile A: 100W x 50H 繼承正確, R=300mm, 切線端點=125mm, 自動對接間距=${_.toExponential(2)}mm)` : `FAIL: W/H inheritance or mate failed (mateDist=${_}mm)`,
			details: {
				straightW: i,
				straightH: a,
				elbowW: s,
	...
	static testCaseX_CatalogProfileB() {
		let e = b.get("VENTILATED_PROFILE_B");
		if (!e) return {
			id: "Case X",
		...
		let t = x("TRAY_STRAIGHT", e), n = x("FITTING_ELBOW_90", e, { angleDeg: 30 }), r = x("FITTING_RISER_OUT_90", e), i = t.effectiveParameters.width === 300, a = t.effectiveParameters.depth === 100, s = n.effectiveParameters.width === 300, c = n.effectiveParameters.angleDeg === 30, l = r.effectiveParameters.width === 300, u = r.effectiveParameters.depth === 100, d = O.validateConnection(t, "PORT_B", n, "PORT_A"), f = D.computePlacement(t, "PORT_B", n, "PORT_A");
		n.setPlacement(f);
		let p = t.getWorldPorts().find((e) => e.id === "PORT_B"), m = n.getWorldPorts().find((e) => e.id === "PORT_A"), h = o.distance(p.worldPosition, m.worldPosition), g = i && a && s && c && l && u && d.valid && h < .001;
		return {
			id: "Case X",
			...
			actual: g ? `PASS (Profile B: 300W x 100H 繼承正確, 30° 彎頭成功對接直槽, 間隙=${h.toExponential(2)}mm)` : `FAIL: W/H inheritance or mate failed (mateDist=${h}mm)`,
			details: {
				straightW: i,
				straightH: a,
				elbowW: s,
	...
	static testCaseY_HorizontalCross() {
		...
		let t = new s("cross_test", e, {
			width: 600,
			depth: 100,
			length: 1450,
			radius: 300,
			tangentLength: 125
		}), n = t.getWorldPorts(), r = t.getCenterlines(), i = t.getBounds(), a = t.getThreeMesh(), c = n.length === 4, l = new Set(n.map((e) => e.id)), u = [
			"PORT_A",
			"PORT_B",
			"PORT_C",
			"PORT_D"
		].every((e) => l.has(e)), d = r.length === 6, f = new Map(n.map((e) => [e.id, e])), p = 0;
		r.forEach((e) => {
			let t = f.get(e.fromPort), n = f.get(e.toPort);
			if (t && n) {
				let r = o.distance(e.samplePoints[0], t.worldPosition), i = o.distance(e.samplePoints[e.samplePoints.length - 1], n.worldPosition);
				p = Math.max(p, r, i);
			}
		});
		let m = i.min[0] === -725 && i.max[0] === 725 && i.min[2] === -725 && i.max[2] === 725, h = a.children.length > 0, _ = c && u && d && p < .001 && m && h;
		return {
			id: "Case Y",
			...
			actual: _ ? `PASS (FITTING_CROSS 4 個埠位對稱齊備, 6 條中心線端點重合誤差=${p.toExponential(2)}mm, 包絡寬度 1450mm)` : `FAIL: ports=${n.length}, routes=${r.length}, maxClError=${p}mm, spanCorrect=${m}`,
			details: {
				ports: n.map((e) => ({
					id: e.id,
					pos: e.worldPosition
				})),
				routesCount: r.length,
				bounds: i
			}
		};
	}
	static testCaseZ_GenericAngleVendor() {
		let e = g.get("FITTING_ELBOW_90"), t = [
			30,
			45,
			60,
			90
		...
		t.forEach((t) => {
			let r = new s(`elbow_${t}`, e, {
				angleDeg: t,
				radius: 600,
				width: 600,
				depth: 100
			}), i = r.getWorldPorts().find((e) => e.id === "PORT_B"), a = t * Math.PI / 180, o = 600 * Math.cos(a), c = -600 * Math.sin(a), l = Math.hypot(i.worldPosition[0] - o, i.worldPosition[2] - c), u = r.getCenterlines()[0], d = 600 * a, f = Math.abs(u.analyticLength - d);
			(l > .01 || f > .01) && n.push({
				angle: t,
				posError: l,
				lenError: f
		...
		return {
			...
			actual: r ? "PASS (30°, 45°, 60°, 90° 四組型錄角度全數經單一通用幾何演算法精確推導, 無重複幾何函式)" : `FAIL: ${n.length} angles failed coordinate derivation`,
			details: {
				catalogAngles: t,
				failures: n
			}
	...
	static testCaseAA_VendorDimensionFormulas() {
		...
		return {
			...
			actual: e ? "PASS (梯型整體寬 W+26, 蓋板 W+38/W+48, 沖底蓋板 W+6, 三通與四通全展開跨距及異徑 600mm 三段過渡全數相符)" : "FAIL: Dimension formula mismatch detected",
			details: {
	...
	static testCaseAB_ProfileConsistencyInvariant() {
		let e = b.getAll(), t = e.length >= 3, n = 0;
		e.forEach((e) => {
			let t = !!e.vendor && e.vendor.length > 0, r = !!e.source && !!e.source.document && e.source.document.length > 0, i = Array.isArray(e.source.pages) && e.source.pages.length > 0, a = e.width > 0 && e.height > 0 && e.standardLength > 0 && e.defaultRadius > 0, o = Array.isArray(e.allowedAngles) && e.allowedAngles.length > 0, s = !!e.coverWidthFormula && e.coverWidthFormula.length > 0;
			t && r && i && a && o && s && n++;
		});
		let r = t && n === e.length;
		return {
			...
			actual: r ? `PASS (全數 ${n}/${e.length} 組規格集具備完整型錄頁碼出處、材料標註、覆蓋公式與允許角度)` : `FAIL: Incomplete profiles detected (${n}/${e.length} valid)`,
			details: {
				totalProfiles: e.length,
				validProfiles: n
			}
		};
	}
...
//#endregion
export { N as AcceptanceTestSuite, c as AnalyticLength, v as AssemblyRegistry, t as AssumptionLevel, k as BomManager, n as BomScope, _ as CatalogStandards, s as ComponentInstance, r as ComponentOrigin, g as ComponentRegistry, i as ComponentRole, O as ConnectionValidator, d as GeometryGenerators, j as JsonExporter, M as LEGACY_FITTING_BASELINES, D as MateEngine, u as Materials, l as RouteGenerator, w as SUB_CATEGORY_NAMES, S as TRAY_SYSTEM_METAS, o as Transforms, b as TraySystemProfiles, a as Units, y as VENDOR_PROFILES, P as VERSION, A as computeGeometryBounds, x as createComponentFromProfile, C as getComponentSubCategory, T as getComponentSystemCategories };
